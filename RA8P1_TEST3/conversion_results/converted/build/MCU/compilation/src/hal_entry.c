...
void hal_entry(void)
{
    ...
    for (int n = 0; n < ntimes; n += 1) {
        memcpy(GetModelInputPtr_serving_default_input_1_0(), model_serving_default_input_1_0, model_serving_default_input_1_0_SIZE);
    ...
    // Comparing outputs
    {
        ...
        int misses = CompareOutput(model_StatefulPartitionedCall_0_70018, GetModelOutputPtr_StatefulPartitionedCall_0_70018(), model_StatefulPartitionedCall_0_70018_COUNT, 0.1f);
        if (misses == 0) {
            SEGGER_RTT_WriteString(0, "MATCH\r\n");
        } else {
            SEGGER_RTT_printf(0, "FAILED: %d/%d misses\r\n", misses, model_StatefulPartitionedCall_0_70018_COUNT);
        }
    }
