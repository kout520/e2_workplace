...
static const st_ov_reg_t cam_config_table_normal_mode[] =
{
 ...
 { 0x3017, 0xFF }, // DVP mode: set Frex, Vsync, Href, PCLK, D[9:6] output
 { 0x3018, 0xFF }, // DVP mode: set D[5:0], GPIO[1:0] output
 { 0x3034, 0x1A }, // DVP 8-bit mode, PCLK normal polarity
 ...
 { 0x300e, 0x45 }, // DVP mode: IO drive strength 2x, DVP PCLK/HREF/VSYNC/D[9:0] output
...
fsp_err_t camera_init (bool use_test_mode)
{
    ...
    // MIPI virtual channel set removed — not needed for DVP mode
    ...
    if(FSP_SUCCESS == fsp_status)
    {
        // DVP mode: open CEU directly, skip MIPI stream_off/on (register 0x4202 is MIPI-only)
        fsp_status = R_CEU_Open(&g_ceu0_ctrl, &g_ceu0_cfg);
    }
...
// CEU capture buffer: 640x480 YCbCr422 = 614400 bytes
#define CAM_FRAME_BYTES (640 * 480 * 2)
static uint8_t ceu_capture_buffer[CAM_FRAME_BYTES] BSP_ALIGN_VARIABLE(128);

void camera_capture_start(void)
{
    fsp_err_t err = R_CEU_CaptureStart(&g_ceu0_ctrl, ceu_capture_buffer);
    // Debug: print CEU start result
    if (FSP_SUCCESS != err) {
        // error — but we can't print here easily, use LED or callback
        volatile uint32_t dbg = err;
        (void)dbg;
    }
}
...
volatile uint32_t g_ceu_last_event = 0;

void g_ceu0_user_callback(capture_callback_args_t *p_args)
{
    ceu_event_t event = (ceu_event_t) p_args->event;
    g_ceu_last_event = (uint32_t) event;
    g_ceu_callback_count++;

    BaseType_t xHigherPriorityTaskWoken, xResult;
    xHigherPriorityTaskWoken = pdFALSE;

    if (CEU_EVENT_FRAME_END == event)
    {
        application_processing_time.camera_image_capture_time_ms = TimeCounter_CountValueConvertToMs(last_ceu_frame_end, TimeCounter_CurrentCountGet());
        last_ceu_frame_end = TimeCounter_CurrentCountGet();
