...
uint8_t gFiveKeyFunc = 0xFF;	//五向按键值
/*********************************************************************/
...
short temp;								//温度
#endif
unsigned long gSportStep = 0;	//运动步数
unsigned long gSportTime = 0;	//运动时间
//初始化过程中相关参数
struct hal_s {
...
//    sprintf(test8,"sys_date:%04d %02d %02d  ", gSystemDate.Year+2000, gSystemDate.Month, gSystemDate.Date);
//    GuiShowString(0, 0, WHITE, BLACK, test8);
//        sprintf(test8,"sys_time:%02d %02d %02d  ", gSystemTime.Hours, gSystemTime.Minutes, gSystemTime.Seconds);
//    GuiShowString(0, 20, WHITE, BLACK, test8);
	#ifdef DEBUG_MODE	
...
void Update_FiveKey_Value(void)
{
    ...
    if((gTaskStateBit.ADCC) && (gTaskStateBit.FiveKeyPress))	//DMA采集完成与五向键按下
    {
        ...
        //根据取值范围，赋予按键功能
        if(gFiveKeyVal >= 0 && gFiveKeyVal < 320)	//0V-0.25V,中间
        {
              gFiveKeyFunc = FIVEKEY_ENSURE;
        } 
        else if(gFiveKeyVal >= 320 && gFiveKeyVal < 960)	//0.25V-0.75V,向下
        {
              gFiveKeyFunc = FIVEKEY_DOWN;		
        }
        else if(gFiveKeyVal >= 960 && gFiveKeyVal < 1600)	//0.725V-1.25V,向左
        {
                gFiveKeyFunc = FIVEKEY_LEFT;
        }
        else if(gFiveKeyVal >= 1600 && gFiveKeyVal < 2240)	//1.25V-1.75V,向上
        {
            gFiveKeyFunc = FIVEKEY_UP;		
        }
        else if(gFiveKeyVal >= 2240 && gFiveKeyVal < 2880)	//1.75V-2.25V,向右
        {
                gFiveKeyFunc = FIVEKEY_RIGHT;		
        }
        else 
        {
                gFiveKeyFunc = 0XFF;	//释放状态
        }
        //#ifdef DEBUG_MODE	
...
void mpu_init_dmp(void)
{
   ...
   */
	dmp_load_Fail = dmp_load_motion_driver_firmware();	//加载DMP固件
	while(dmp_load_Fail)	
	{
		dmp_load_Fail = dmp_load_motion_driver_firmware();	
		HAL_Delay(100);
	}
   ...
   hal.dmp_features = DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
       ...
       DMP_FEATURE_GYRO_CAL | DMP_FEATURE_PEDOMETER;	//DMP的功能开启选项标志位设置
 ...
 void mpu_get_max30102_data(void)
{
  int16_t i;
	uch_dummy = 0x00;  
    uint8_t temp_config = 0x01;
    maxim_max30102_write_reg(REG_TEMP_CONFIG, temp_config);
  ...
  n_temperature = integer_temperature + ((float)fractional_temperature)/16.0 + 39.7f;
