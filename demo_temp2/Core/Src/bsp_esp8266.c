...
_Bool ESP8266_Read_IP(void)
{
  ...
  char mac[40] = {0};
		reset_rxbuffClear(); 
  //HAL_UART_Transmit(&huart5, cCmd, strlen((char*)cCmd), 0xffff); // 发送AT指令
  ESP8266_Send_AT_Cmd( (char *)cCmd, "OK", NULL, 1000 );
	HAL_Delay(200);
	ESP8266_Fram_Record_Struct .Data_RX_BUF[ESP8266_Fram_Record_Struct.InfBit.FramLength]='\0';
	memcpy(ipv4,ESP8266_Fram_Record_Struct .Data_RX_BUF + 39, 16);
	memcpy(StationIPV4, ipv4, strlen(ipv4)-1);
	ESP8266_Fram_Record_Struct.InfBit.FramLength=0;
	/*
	//从接收结果中取出IP
	i = sscanf((char*)ESP8266_Fram_Record_Struct .Data_RX_BUF, "AT+CIFSR\r\n+CIFSR:STAIP,\"%s\"\r\n+CIFSR:STAMAC,\"%s\"\r\n\r\nOK", 
				ipv4, mac);
	if(i > 0)
	{
		memcpy(StationIPV4, ipv4, strlen(ipv4)-1);
...
int ESP8266UdpTest()
{
	...
	{
		GuiShowString(10, 10, BLACK, RED,"cannot connect");
		HAL_Delay(1000);
	}
	//GuiShowString(10, 10, BLACK, RED,"connect suc");
			HAL_Delay(1000);
	...
	//GuiShowString(10, 10, BLACK, RED,(char *)StationIPV4);
	printf("\r\n进入透传发送：\r\n");
	...
	//GuiShowString(10, 10, BLACK, RED,"send enable");
	memset(ESP8266_Fram_Record_Struct.Data_RX_BUF, 0, sizeof(ESP8266_Fram_Record_Struct.Data_RX_BUF));
	return 1;
	/*
	while(1)
	{
		sprintf (str,"huaqingyuanjian\r\n" );//格式化发送字符串到TCP服务器
		ESP8266_SendString ( ENABLE, str, 0, Single_ID_0 );
		HAL_Delay(1000);	
		if(ESP8266_Fram_Record_Struct.InfBit.FramLength > 0)
		{
			ESP8266_Fram_Record_Struct.Data_RX_BUF[ESP8266_Fram_Record_Struct.InfBit.FramLength] = '\0';
			sprintf(str, "%s         ", ESP8266_Fram_Record_Struct.Data_RX_BUF);
			GuiShowString(10, 30, BLACK, RED,str);
			ESP8266_Fram_Record_Struct.InfBit.FramLength = 0;
			//memset(ESP8266_Fram_Record_Struct.Data_RX_BUF, 0, sizeof(ESP8266_Fram_Record_Struct.Data_RX_BUF));
		}
	}
...
void ESP8266_STA_MQTTClient_Test(void)
{
    ...
    while(!ESP8266_JoinAP(User_ESP8266_SSID, User_ESP8266_PWD));
	  ...
	  ESP8266_MQTTSUB( User_ESP8266_MQTTServer_Topic);
		...
		while(1)
		{
				sprintf(str,"1");//格式化发送字符串到MQTT服务器
