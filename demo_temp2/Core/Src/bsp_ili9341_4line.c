...
**********************************************************************
* @fun     : LCD_ShowImage
* @brief   : 在指定坐标显示 RGB565 格式图像
* @param   : x,y     : 图像左上角坐标
*            width   : 图像宽度（像素）
*            height  : 图像高度（像素）
*            image   : 指向 RGB565 图像数据的指针
* @return  : None
**********************************************************************
*/
...
{
    /* 参数合法性检查，防止越界 */
    if ((x + width)  > ILI9341dev.width)  return;
    if ((y + height) > ILI9341dev.height) return;
    if (image == NULL) return;

    uint32_t total_bytes = (uint32_t)width * height * 2; /* RGB565: 2字节/像素 */
    uint32_t sent        = 0;
    const uint32_t CHUNK = 4096; /* 每次SPI传输的字节数，可根据RAM调整 */

    /* 强制刷新区域缓存，确保 SetArea 正确写入 */
    old_x0 = 0xFFFF; old_x1 = 0xFFFF;
    old_y0 = 0xFFFF; old_y1 = 0xFFFF;

    /* 设定显示区域 */
    ILI9341_SetArea(x, y, x + width - 1, y + height - 1);

    /* 发送写 GRAM 命令 */
    ILI9341_WriteRAM_Prepare();

    /* 分块传输图像数据（避免单次传输过大导致超时） */
    while (sent < total_bytes)
    {
        uint32_t size = ((total_bytes - sent) > CHUNK)
                        ? CHUNK
                        : (total_bytes - sent);
...
const unsigned char   Zk_ASCII8X16[]=
 {
