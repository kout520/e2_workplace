...
void hal_entry(void)
{
    ...
    for (int n = 0; n < ntimes; n += 1) {
        memcpy(GetModelInputPtr_serving_default_keras_tensor_121_0(), model_serving_default_keras_tensor_121_0, model_serving_default_keras_tensor_121_0_SIZE);
    ...
    // Comparing outputs
    {
        ...
        int misses = CompareOutput(model_StatefulPartitionedCall_1_0_70408, GetModelOutputPtr_StatefulPartitionedCall_1_0_70408(), model_StatefulPartitionedCall_1_0_70408_COUNT, 0.1f);
        if (misses == 0) {
            SEGGER_RTT_WriteString(0, "MATCH\r\n");
        } else {
            SEGGER_RTT_printf(0, "FAILED: %d/%d misses\r\n", misses, model_StatefulPartitionedCall_1_0_70408_COUNT);
        }
    }
