...
typedef struct
{
	uint32_t UPDATE_DIAL_EN:1;					//表盘页面任务使能			
	...
	uint32_t UPDATE_APPPAGE:1;		      //应用界面使能
...
//系统运行指示灯与扩展指示灯LD1
#define RUN_LED_ENABLE()				HAL_GPIO_WritePin(RUN_LED_GPIO_Port, RUN_LED_Pin, GPIO_PIN_SET)		 				/* 使能运行指示灯 */
#define RUN_LED_DISABLE()	  		HAL_GPIO_WritePin(RUN_LED_GPIO_Port, RUN_LED_Pin, GPIO_PIN_RESET)					/* 禁止运行指示灯 */
#define RUN_LED_TURN_STATE()	  HAL_GPIO_TogglePin(RUN_LED_GPIO_Port, RUN_LED_Pin)												/* 禁止运行指示灯 */
//扩展板指示灯LD2
...
//五向按键状态
#define FIVEKEY_UP			4
#define FIVEKEY_DOWN		3
#define FIVEKEY_LEFT		2
#define FIVEKEY_RIGHT		1
#define FIVEKEY_ENSURE	0
//
